...
class Database:
    def __init__(self):
        ...
        try:
            ...
            self.logger.info('Failed to conect or create database. \
            Both could be in existance')
    ...
    def createTable(self):
        try:
            ...
            self.logger.warning('FAILED TO CREATE THE TABLE.\
    ...
    def selectCountAll(self):
        try:
            ...
            self.logger.info('{} Total record count from db \
                '.format(totalRecords))
            return totalRecords
        except Exception:
            self.logger.warning('UNABLE TO SELECT ALL RECORDS FROM DATABASE')
    ...
    def selectRecentGames(self, records):
        try:
            ...
            query = 'select * from EasyChess order by created_at desc limit \
                 {}'.format(records)
            self.cursor.execute(query)
            self.logger.info('{} most recent game records selected from db \
                '.format(records))
            return self.cursor.fetchall()
        except Exception:
            self.logger.warning('UNABLE TO SELECT MOST RECENT RECORDS FROM DATABASE \
                 WITH LIMIT {}'.format(records))
            return -1
    ...
    def selectRecentWins(self, records):
        try:
            ...
            query = 'select * from EasyChess where won <> 0 order by \
                created_at desc limit {}'.format(records)
            self.cursor.execute(query)
            self.logger.info('Recent game wins selected from db with \
                limit {}'.format(records))
            return self.cursor.fetchall()
        except Exception:
            self.logger.warning('UNABLE TO SELECT MOST RECENT WINS FROM \
                DATABASE WITH LIMIT {}'.format(records))
            return -1
    ...
    def selectRecentLosses(self, records):
        try:
            query = 'select * from EasyChess where won = 0 order by \
                created_at desc limit {}'.format(records)
            self.cursor.execute(query)
            self.logger.info('Recent game losses selected from db \
                with limit {}'.format(records))
            return self.cursor.fetchall()
        except Exception:
            self.logger.warning('UNABLE TO SELECT MOST RECENT WINS \
                FROM DATABASE WITH LIMIT {}'.format(records))
            return -1
    ...
    def selectTopWins(self, records):
        try:
            ...
            query = 'select * from EasyChess where won <> 0 order \
                by moves DESC limit {}'.format(records)
            self.cursor.execute(query)
            self.logger.info('Top wins with limit {} selected from \
                db'.format(records))
            return self.cursor.fetchall()
        except Exception:
            self.logger.warning('UNABLE TO GET TOP WINS FROM THE \
                DATABASE WITH LIMIT {}'.format(records))
            return -1
    ...
    def selectTopLosses(self, records):
        try:
            ...
            query = 'select * from EasyChess where won = 0 order \
                by moves limit {}'.format(records)
            self.cursor.execute(query)
            self.logger.info('Top losses with limit {} selected \
                from db'.format(records))
            return self.cursor.fetchall()
        except Exception:
            self.logger.warning('UNABLE TO GET TOP LOSSES FROM THE \
                DATABASE WITH LIMIT {}'.format(records))
            return -1
