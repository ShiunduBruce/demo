        ...
        if self.in_board:
            ...
            for x,y in move:
                ...
                while not collision and self.is_inside_border(curr_pos):
                    ...
                    if curr_pos == target:
                        if self.get (target, board) == 0 or self.get (position, board).color != self.get (target, board).color:
                            return True
                        return False
...
class Knight(Figure):
    ...
    def check_move_possible(self, position, target, board):
        if self.in_board:
            l_move  = ((1,2),(2,-1),(-1,-2),(-2,1),(-1,2),(2,1),(1,-2),(-2,-1))
...
class Pawn(Figure):
    ...
    def check_move_possible(self, position, target, board):
        move = ((1 if self.is_reverse else -1, 0), (1 if self.is_reverse else -1, -1), (1 if self.is_reverse else -1, 1))
        ...
        if self.in_board:
            ...
            for i in (1, 2):
                ...
                if self.is_inside_border(curr_pos) and curr_pos == target:
                    if self.get (target, board) != 0 and self.get (position, board).color != self.get (target, board).color:
                        self.check_reverse(target[0])
                        return True
                    return False
